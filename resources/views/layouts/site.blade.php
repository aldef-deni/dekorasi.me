@php
    ...
    $metaTitle = trim($__env->yieldContent('meta-title')) ?: ($siteName . ' — ' . setting('site.tagline', 'Desain Interior Premium'));
    $metaDesc  = trim($__env->yieldContent('meta-description')) ?: setting('site.description', 'Studio desain interior yang mengubah ruang menjadi pengalaman.');
@endphp
<!DOCTYPE html>
<html lang="id">
<head>
...
<body>
  ...
  @if (setting('contact.whatsapp'))
    <a class="wa-float" href="{{ whatsapp_url(setting('contact.whatsapp'), 'Halo ' . $siteName . ', saya ingin konsultasi desain interior.') }}"
       target="_blank" rel="noopener" aria-label="Hubungi via WhatsApp">
